...
const onNavigate = (pathname) => {
  ...
  if (pathname === "/") {
    renderFormData();
    document.getElementById("homeBtn").classList.add("nav-item-active");
    document.getElementById("registerBtn").classList.remove("nav-item-active");
  } else if (pathname === "/register") {
    document.getElementById("registerBtn").classList.add("nav-item-active");
    document.getElementById("homeBtn").classList.remove("nav-item-active");
  }
};
...
function renderFormData() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    cell4.innerHTML = ('+370 ') + item.phone;
    //email
    ...
    cell7.innerHTML = `<a class="btn btn-edit" onClick="onNavigate('/register');onEdit(this);">Edit</a>
                         <a class="btn btn-delete" onClick="onDelete(this)">Delete</a>`;
...
function validateUnique() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (document.getElementById("phone").value === item.phone) {
      ...
      if (
        !document.getElementById("phoneValidation").classList.contains("hide")
      )
      ...
      if (document.getElementById("email").value === item.email) {
        ...
        if (
          !document.getElementById("emailValidation").classList.contains("hide")
        )
